...
export default function BlogManager() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        setModal({ isOpen: true, title: "Success", message: "Blog updated successfully!", type: "success" });
      } else {
        ...
        setModal({ isOpen: true, title: "Success", message: "Blog added successfully!", type: "success" });
      }
      resetForm();
      fetchBlogs();
    } catch (error) {
      console.error("Error:", error);
      setModal({ isOpen: true, title: "Error", message: "Failed to save blog. Please try again.", type: "error" });
    }
  };
  ...
  const confirmDelete = async () => {
    try {
      ...
      setModal({ isOpen: true, title: "Success", message: "Blog deleted successfully!", type: "success" });
      fetchBlogs();
    } catch (error) {
      console.error("Error:", error);
      setModal({ isOpen: true, title: "Error", message: "Failed to delete blog. Please try again.", type: "error" });
    }
    setDeleteId(null);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {blogs.map((blog) => (
          <div key={blog._id} className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10">
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => handleDelete(blog._id)} className="p-2 bg-red-600 hover:bg-red-700 text-white rounded-lg">
                  <FaTrash />
                </button>
              </div>
            </div>
